...
get_gsea_config = function(object,case,control){
    ...
    cells = rownames(object@meta.data)[object@meta.data$group %in% c(case,control)]
    ...
    cls$col1 = as.character(cls$col1)
    cls$col1[1] = length(object@meta.data$group)
    cls$col1[2] = 2
    cls$col1[3] = 1

    cls$col2 = as.character(cls$col2)
    cls$col2[1] = '#'
    cls$col2[2] = sy
    cls$col2[3] = dz
    ...
    write.table(cls,file=outname,sep=' ',quote=F,col.names=F,row.names=F)
