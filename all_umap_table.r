library(Seurat)
data_ob=OESingleCell::ReadX(input = 'add_yaqun.h5seurat', informat = 'h5seurat', verbose = F)
...
ggumap = DimPlot(object = data_ob, reduction = "umap", pt.size =0.5, group.by="yaqun_clusters") +
    ggplot2::theme( plot.title = ggplot2::element_text(hjust = 0.5)) +
    ggplot2::scale_colour_manual( values = CustomCol2(1:35)) + ggplot2::coord_fixed(ratio = 1)
ggplot2::ggsave(file.path("./",paste0("yaqun_clusters",".pdf")), ggumap,width=8)
system(glue::glue("convert  -verbose -density 500 -trim  yaqun_clusters.pdf  -quality 100  -flatten  yaqun_clusters.png"))
