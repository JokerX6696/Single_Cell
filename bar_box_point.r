...
group1 <- annos[grepl("Cytoledon_enbryo-vs-Torpedo_embryo",annos)]
group2 <- annos[!grepl("Cytoledon_enbryo-vs-Torpedo_embryo",annos)]
###################################################################################  group1
...
dfcol <- data.frame(
  ...
  y = 0,
  label = c(1:length(unique(df$cluster)))
)
...
################################################################################### group2
group_vs <- strsplit(group2[1],"/")[[1]][2]
treat <- strsplit(group_vs,"-vs-")[[1]][1]
con <- strsplit(group_vs,"-vs-")[[1]][2]
df = data.frame()
for(group in group2){
